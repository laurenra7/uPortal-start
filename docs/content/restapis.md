# Add Custom REST APIs to uPortal

Web components for uPortal will often need to call APIs that require information
about the user, like user ID. You may need to create a custom API for your
institution that

To add custom user attributes to IPerson so they are available to the REST
API, [see here](#).

(Task: Build a general-purpose API to decorate requests to other APIs with
uPortal attributes, like person attributes?)
